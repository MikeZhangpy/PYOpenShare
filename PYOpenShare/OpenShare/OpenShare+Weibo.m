...
+(void)shareToWeibo:(OSMessage*)msg Success:(shareSuccess)success Fail:(shareFail)fail{
    ...
    }else if ([msg isEmpty:@[@"shareLinkTitle"] AndNotEmpty:@[@"title",@"link" ,@"image"] ]) {
        //链接类型分享
        message=@{
                  @"__class" : @"WBMessageObject",
                  @"mediaObject":@{
    ...
    }else if ([msg isEmpty:nil AndNotEmpty:@[@"shareLinkTitle",@"title",@"link" ,@"image"]]) {
        message=@{
                  @"__class" : @"WBMessageObject",
                  @"imageObject":@{
                          @"imageData":msg.image,
                          },
                  @"text" : msg.shareLinkTitle
                  };
    }
